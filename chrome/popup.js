...
builtinToggle.addEventListener('change', () => {
  chrome.storage.local.set({ builtinEnabled: builtinToggle.checked });
  loadStats();
  loadSites();
});
...
updateBtn.addEventListener('click', () => {
  ...
  chrome.runtime.sendMessage({ action: 'updateBlocklist' }, (response) => {
...
async function loadSites() {
  const { blockedSites, builtinEnabled: enabled } = await chrome.storage.local.get(['blockedSites', 'builtinEnabled']);
